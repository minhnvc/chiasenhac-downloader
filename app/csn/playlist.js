...
module.exports = class Playlist {
	...
	async start() {
		...
		console.log("Total songs : " + numOfSong)

		const regex = /playlist\/(.*).html/g
		...
		for (let i = 1; i <= numOfSong; i++) {
			...
			let anchors = song("#download_lossless")
			if (anchors.length > 0) {
				for (let index = 0; index < anchors.length; index++) {
					arrayDownload.push(anchors[index].attribs.href)
				}
			} else {
				anchors = song(".download_item")
				for (let index = anchors.length - 1; index >= 0; index--) {
					if (anchors[index].attribs.href.substring(anchors[index].attribs.href.length - 3) === "mp3") {
						arrayDownload.push(anchors[index].attribs.href)
						break
					}
				}
